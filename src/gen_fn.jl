...
generated by the language model. The return value is also the completion output.
...
function Base.getindex(trace::GPT3Trace, addr)
    if addr == :prompt
        return trace.prompt
    elseif addr == :tokens
        return trace.tokens
...
function generate(gen_fn::GPT3GF, args::Tuple, constraints::ChoiceMap)
    ...
    if isnothing(full_text)
        score = -Inf
        trace = GPT3Trace(gen_fn, prompt, output, String[], Float64[], score)
        return trace, score
    end
