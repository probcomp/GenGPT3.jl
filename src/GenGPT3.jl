...
Constructs a static choicemap for the output of a [`GPT3GenerativeFunction`](@ref).
"""
...
function simulate(gen_fn::GPT3GF, args::Tuple)
    ...
    # Compute log probability of generating output (including stop sequence)
    stop_idx = find_stop_index(completion, gen_fn.n_stop)
    tokens = completion.logprobs.tokens[1:stop_idx]
    if gen_fn.temperature == 0.0
        logprobs = zeros(Float64, length(tokens))
    else
        logprobs = completion.logprobs.token_logprobs[1:stop_idx]
        logprobs ./= gen_fn.temperature
    end
    score = isempty(logprobs) ? 0.0 : sum(logprobs)

    # Return trace
    return GPT3Trace(gen_fn, prompt, output, tokens, logprobs, score)
end
...
function generate(gen_fn::GPT3GF, args::Tuple, constraints::ChoiceMap)
    ...
    output_tokens = id_tokenize(output)
    n_output = length(output_tokens)
    if n_output + gen_fn.n_stop <= gen_fn.max_tokens # Finish due to stop words
    ...
    elseif n_output == gen_fn.max_tokens # Finish due to length
        # Construct full text from prompt and output
        full_text = prompt * output
    else # Return impossible trace
        score = -Inf
        return GPT3Trace(gen_fn, prompt, output, [], [], score), score
    end
    ...
    if gen_fn.temperature == 0.0
        ...
        logprobs = collect(Float64, completion.logprobs.token_logprobs[start_idx:end])
        logprobs ./= gen_fn.temperature
    end
    score = isempty(logprobs) ? 0.0 : sum(logprobs)

    # Return trace
    return GPT3Trace(gen_fn, prompt, output, tokens, logprobs, score), score    
end
...
project(trace::GPT3Trace, selection::Selection) =
    has_value(selection, OUTPUT_ADDR) ? trace.score : 0.0   
...
function regenerate(trace::GPT3Trace, args::Tuple, argdiffs::Tuple, selection::Selection)
    ...
    # Simulate new trace and return change in score
